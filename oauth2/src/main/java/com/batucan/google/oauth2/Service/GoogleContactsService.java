...
public class GoogleContactsService {
    ...
    public void updateContact(String resourceName, Contact contact, String accessToken) {
        try {
            ...
            Map<String, Object> requestBody = new HashMap<>();
            requestBody.put("etag", contact.getEtag());
            requestBody.put("names", List.of(Map.of("givenName", contact.getName())));
            requestBody.put("emailAddresses", contact.getEmails().stream().map(email -> Map.of("value", email)).toList());
            requestBody.put("phoneNumbers", contact.getPhoneNumbers().stream().map(phone -> Map.of("value", phone)).toList());
            ...
            if (response.getStatusCode() == HttpStatus.OK) {
                System.out.println("✅ Successfully updated contact with ID: " + resourceName);
            } else {
                System.err.println("❌ Failed to update contact. Status: " + response.getStatusCode());
            }
        } catch (Exception e) {
